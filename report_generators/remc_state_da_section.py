'''
create REMC State Day Ahead Forecast summary Section data in the column sequence
name,max,min,avg
'''
import pandas as pd
...
def populateRemcStateDaSummSectionData(configFilePath, configSheetName, outputFilePath, outputSheetName, truncateSheet=False):
    sectionDataDf = getRemcStateDaSummSectionDataDf(
...
def getRemcStateDaSummSectionDataDf(configFilePath, configSheetName):
    # get conf dataframe
    ...
    for rowIter in range(confDf.shape[0]):
        ...
        printWithTs(
            'REMC State Day Ahead Forecast summary processing row number {0}'.format(rowIter+2))

        # get the type of row, itcan be dummy / normal / agg_gen_type2 / agg_gen_type
